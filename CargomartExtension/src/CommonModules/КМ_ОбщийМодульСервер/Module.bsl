...
Функция СохранитьОбновитьПунктВ_БД(Организация, СтрокаКВыгрузке, Результат, НайденыеКЛкЗаписи = Неопределено, НайденыеКЛкУдалению = Неопределено)
	...
	Если СтрокаКВыгрузке.ДанныеКМ.Свойство("contact")
		...
		Для Каждого Элемент Из СтрокаКВыгрузке.ДанныеКМ.contact Цикл
			...
			Результат.Лог = Результат.Лог + "Контактное лицо " + Строка(?(Элемент.Свойство("name"), Элемент.name, "")) + " в Пункт """ 
...
Функция ВыгрузитьПунктВКаргомарт(СоединениеHTTP, Токен, СтрокаКВыгрузке, Организация, Результат, ТаблицаСоответствияКонтактныхЛиц = Неопределено)
    ...
    Если ЗначениеЗаполнено(СтрокаКВыгрузке.НаселенныйПункт) Тогда
		ДанныеЗапроса.Вставить("localityname", СтрокаКВыгрузке.НаселенныйПункт);
	КонецЕсли;

    Если ЗначениеЗаполнено(СтрокаКВыгрузке.КЛАДР) Тогда
		ДанныеЗапроса.Вставить("code", СтрокаКВыгрузке.КЛАДР);
	КонецЕсли;

    Если ЗначениеЗаполнено(СтрокаКВыгрузке.Адрес) Тогда
		ДанныеЗапроса.Вставить("address", СтрокаКВыгрузке.Адрес);
	КонецЕсли;

    Если ЗначениеЗаполнено(СтрокаКВыгрузке.НомерДома) Тогда
		ДанныеЗапроса.Вставить("house", СтрокаКВыгрузке.НомерДома);
	КонецЕсли;

    Если ЗначениеЗаполнено(СтрокаКВыгрузке.КоординатыШирота) и СтрокаКВыгрузке.КоординатыШирота > 0 Тогда
		ДанныеЗапроса.Вставить("latitude", СтрокаКВыгрузке.КоординатыШирота);
	КонецЕсли;

    Если ЗначениеЗаполнено(СтрокаКВыгрузке.КоординатыДолгота) и СтрокаКВыгрузке.КоординатыДолгота > 0 Тогда
		ДанныеЗапроса.Вставить("longitude", СтрокаКВыгрузке.КоординатыДолгота);
	КонецЕсли;

    Если ЗначениеЗаполнено(СтрокаКВыгрузке.ФормаОрганизации) Тогда
		ДанныеЗапроса.Вставить("legalStatus", СтрокаКВыгрузке.ФормаОрганизации);
	КонецЕсли;

    Если ЗначениеЗаполнено(СтрокаКВыгрузке.ЮридическоеНаименование) Тогда
		ДанныеЗапроса.Вставить("legalname", СтрокаКВыгрузке.ЮридическоеНаименование);
    КонецЕсли;
...
Функция ДанныеПунктаРазличаются(Строка)
    ...
    Если Не ЗначениеЗаполнено(Строка.КонтактноеЛицо)
    	И Строка.ДанныеКМ.Свойство("contact") 
    	И ТипЗнч(Строка.ДанныеКМ.contact) = Тип("Массив")
    	И Не ЗначениеЗаполнено(Строка.ДанныеКМ.contact)   Тогда
    	
    ИначеЕсли ЗначениеЗаполнено(Строка.КонтактноеЛицо)
    	И Строка.ДанныеКМ.Свойство("contact") 
    	И ТипЗнч(Строка.ДанныеКМ.contact) = Тип("Массив")
    	И ЗначениеЗаполнено(Строка.ДанныеКМ.contact)
    	И Строка.Строки.Количество() = Строка.ДанныеКМ.contact.Количество() Тогда
    	
    	Итр = 0;
    	
    	Для Каждого стр из Строка.Строки Цикл
    		    		
    		Если (стр.КонтактноеЛицо.name = ?(Строка.ДанныеКМ.contact[Итр].Свойство("name"), Строка.ДанныеКМ.contact[Итр].name, "")) 
    			И (стр.КонтактноеЛицо.phone = ?(Строка.ДанныеКМ.contact[Итр].Свойство("phone"), Строка.ДанныеКМ.contact[Итр].phone, "")) Тогда
...
//			ЗаказОбъект.СтавкаНДС 						= ?(ПолученныеДанные.order.Свойство("vatRate"),
//				КМ_ОбщийМодульПовтИсп.СтавкаНДСПоЧислу(ПолученныеДанные.order.vatRate), Перечисления.СтавкиНДС.БезНДС);	
...
Процедура ЗаполнитьСтрокуКонтактнойИнформацииEMail(ТЧКонтактнаяИнформация, ВидКИ, ЗначениеEMail = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ЗначениеEMail) Тогда
		...
		СтрокаКИ.ЗначенияПолей 	= УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияИзJSONВXML(СтрокаКИ.Значение
			, СтрокаКИ.Тип);
...
Функция ОпределитьКонтрагента(ДанныеКонтрагента, ПротоколОшибок = "")
	...
	Иначе
		...
		//Получим ключ МДМ для контрагента.
		
		КонтрагентСтроковоеПредставление = Строка(КонтрагентОбъект);//На всякий, вдруг исключение по обращению к БД?
		
		Попытка //Работа с МДМ реализована в расширении "АдаптацияБП_МДМ". Вызываем модуль расширения через попытку.
			
			РезультатМДМ = Неопределено;
			Выполнить("РезультатМДМ = МАГ_ОбращениеК_МДМ.ЗаполнитьРеквизитыНаСервере(КонтрагентОбъект);");
			
			Если ТипЗнч(РезультатМДМ) = Тип("Структура") 
				И РезультатМДМ.Свойство("ЕстьОшибка")
				И РезультатМДМ.ЕстьОшибка = Истина Тогда
				
				ОбщегоНазначения.СообщитьПользователю("Ошибка исполнения функции ""МАГ_ОбращениеК_МДМ.ЗаполнитьРеквизитыНаСервере"".при создании нового контрагента:" + Символы.ПС + РезультатМДМ.ОписаниеОшибки);
				ОбщегоНазначения.СообщитьПользователю("Контрагент:" + КонтрагентСтроковоеПредставление);
				ОбщегоНазначения.СообщитьПользователю("Результат исполнения: " + Строка(РезультатМДМ));
	 
			КонецЕсли;
			
		Исключение
			ОбщегоНазначения.СообщитьПользователю("Исключение при попытке обращения к МДМ при создании нового контрагента:" + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));				
		КонецПопытки;
