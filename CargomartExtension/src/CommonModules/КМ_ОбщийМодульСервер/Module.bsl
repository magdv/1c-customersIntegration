...
Функция ПолучитьТокенАвторизацииКаргомарт(Организация, НастройкиКаргомарт, СоединениеHTTP, Результат = Неопределено) Экспорт
	...
	ЗапросHTTPАвторизация.УстановитьТелоИзСтроки(КМ_ОбщийМодульКлиентСервер.ЗначениеВJSON(СтруктураАвторизации),
												 КодировкаТекста.UTF8, 
												 ИспользованиеByteOrderMark.НеИспользовать);//"{""login"":""" + Логин +""",""password"": """+ Пароль + """}",,);
