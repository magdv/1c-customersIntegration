...
/////
//Функция КМ_СотрудникиФизическиеЛицоПолучить(Строка, Владелец) Экспорт
//	
//	Результат = Справочники.ФизическиеЛица.НайтиПоРеквизиту("Наименование", Строка, ,Владелец);
//	
//	Возврат ?(Результат = Справочники.ФизическиеЛица.ПустаяСсылка(), Неопределено, Результат);
//	
//КонецФункции
/////
...
Функция ТекстЗапросСотрудники(Параметры) Экспорт
	...
	ТаблицаСоответствия.Колонки.Добавить("Имя", 		Новый ОписаниеТипов("Строка"));
	ТаблицаСоответствия.Колонки.Добавить("Фамилия", 	Новый ОписаниеТипов("Строка"));
	ТаблицаСоответствия.Колонки.Добавить("Отчество", 	Новый ОписаниеТипов("Строка"));
	...
	Для каждого Стр из ТаблицаСоответствия Цикл
		...
		ФИО = СтрРазделить(ФИО, " ");
		
		Стр.Фамилия  	= ФИО[0];
		Стр.Имя 		= ?(ФИО.Количество() > 1, ФИО[1], "");
		Стр.Отчество 	= ?(ФИО.Количество() > 2, ФИО[2], "");
			
	КонецЦикла;
	
	ТаблицаСоответствия.Колонки.Удалить("Наименование");
...
Процедура СформироватьФормуРабочееМесто(Форма) Экспорт
	...
	стрРеквизитов.Вставить("ПодразделениеЗатрат", 				КМ_ОбщийМодульДФИ.ОписаниеТипаСтрока(5));
	стрРеквизитов.Вставить("Субконто1_НГ", 						КМ_ОбщийМодульДФИ.ОписаниеТипаСтрока(5));
	стрРеквизитов.Вставить("Субконто2_СЗ", 						КМ_ОбщийМодульДФИ.ОписаниеТипаСтрока(5));
	стрРеквизитов.Вставить("СтатьяДДСОплатаТранспортныхУслуг", 	КМ_ОбщийМодульДФИ.ОписаниеТипаСтрока(5));
	стрРеквизитов.Вставить("ПодразделениеОрганизации", 			КМ_ОбщийМодульДФИ.ОписаниеТипаСправочник("СтруктураПредприятия"));
	стрРеквизитов.Вставить("СчетЗатрат", 						КМ_ОбщийМодульДФИ.ОписаниеТипаСтрока(5));
	стрРеквизитов.Вставить("СчетДоходовРТУ", 					КМ_ОбщийМодульДФИ.ОписаниеТипаСтрока(5));
	стрРеквизитов.Вставить("СчетРасходовРТУ", 					КМ_ОбщийМодульДФИ.ОписаниеТипаСтрока(5));
	стрРеквизитов.Вставить("СчетУчетаНДС", 						КМ_ОбщийМодульДФИ.ОписаниеТипаСтрока(5));
	стрРеквизитов.Вставить("СчетУчетаНДСПоРеализации", 			КМ_ОбщийМодульДФИ.ОписаниеТипаСтрока(5));
	стрРеквизитов.Вставить("СпособУчетаНДС", 					КМ_ОбщийМодульДФИ.ОписаниеТипаСтрока(5));
	
	КМ_ОбщийМодульДФИ.СоздатьРеквизиты(Форма, стрРеквизитов);
	
	Форма.Субконто1_НГ 						= "УТ";
	Форма.Субконто2_СЗ 						= "УТ";
	Форма.СтатьяДДСОплатаТранспортныхУслуг 	= "УТ";
	Форма.ПодразделениеЗатрат 				= "УТ";
	Форма.СчетЗатрат 						= "УТ";
	Форма.СчетДоходовРТУ 					= "УТ";
	Форма.СчетРасходовРТУ 					= "УТ";
	Форма.СчетУчетаНДС 						= "УТ";
	Форма.СчетУчетаНДСПоРеализации 			= "УТ";
	Форма.СпособУчетаНДС 					= "УТ";
...
Процедура ЗаполнитьДанныеФормыКМ_Сотрудники(Форма) Экспорт
	...
	Запрос = Новый Запрос;
		...
		Форма.Фамилия 				= ФИО[0];
		Форма.Имя 					= ?(ФИО.Количество() > 1, ФИО[1], "");
		Форма.Отчество 				= ?(ФИО.Количество() > 2, ФИО[2], "");
...
Функция СформироватьФормуКМЗаказ(Форма, ДополнительныеПараметры) Экспорт
//	
КонецФункции
...
//Функция заполняет и возвращает Объект Контрагент.
//
//Параметры:
//	ОбъектФЛ 	- СправочникСсылка.Контрагенты 	- Объект Контрагенты
//	ФИО 		- Строка 							- Строка с ФИО
//Возвращаемое значение:
//	СправочникСсылка.Контрагенты - Заполненый объект Контрагента
